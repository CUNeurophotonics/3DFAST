...
def flip_x(device):
    nodemap = device.nodemap
    nodes = nodemap.get_node(['ReverseX'])
    nodes['ReverseX'].value = bool(1)
...
def set_DMD_SIM(dmd,exposure_time_cam):
    ...
    pats = io.imread('patterns//DD20_Stack.tif')
    #pats = io.imread('patterns//DD40_Stack3.tif')                                  #!!!!!!!! EXTRA for Z slices! 
    pats[np.where(pats!=0)] = 1
    # Get DMD & Upload Pats
    #dmd = dlp6500.dlp6500win(debug=False, index = 1)
    exposure_t = exposure_time_cam
    dark_t = 105
    triggered = False
    img_inds, bit_inds = dmd.upload_pattern_sequence(pats, exposure_t, dark_t, triggered, clear_pattern_after_trigger = False, bit_depth = 1, num_repeats = 15)
    print("Upload Complete")
    dmd.set_pattern_sequence(img_inds, bit_inds, exposure_t, dark_t, triggered, clear_pattern_after_trigger= False, bit_depth=1, num_repeats=15, mode='on-the-fly')
    dmd.start_stop_sequence("start")
    print("DMD IS INITIALIZED")
    #return dmd

def set_DMD_TARGET_SIM(dmd,exposure_time_cam,mask):
    dmd.start_stop_sequence("stop")
    # import tif file into numpy array (BMP NOW!!???)
...
#dmd1.set_trigger_in1(104,'rising')
#set_DMD_SIM(dmd1,60000)

#dmd1.set_trigger_out(1,False,0,0)

set_DMD_SIM(dmd1,5000)
